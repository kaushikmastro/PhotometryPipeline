"""
HAMO F1B geometry grind — mission-science Gaskell DSK256 (vesta_gaskell_256_110825.bds).
...
def main() -> None:
    ...
    hamo_f1b = sorted(
        (DATA_ROOT / "01_calibrated_images" / "hamo").glob("*F1B*.IMG")
    )
    logging.info("HAMO F1B images found: %d", len(hamo_f1b))

    if not hamo_f1b:
        logging.error("No HAMO F1B images found under %s",
