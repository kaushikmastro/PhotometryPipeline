...
if [[ ! -f "$DATA_ROOT/spice_kernels/dawn_dynamic.tm" ]]; then
    echo "  ERROR: Dynamic metakernel not found!"
    exit 1
fi

# Check August 2011 SPK coverage
if ! grep -q "110802-110831" "$DATA_ROOT/spice_kernels/dawn_dynamic.tm"; then
...
# Count kernels
NK=$(grep -c "\.bsp\|\.tsc\|\.bc\|\.tf\|\.ti\|\.tpc\|\.tls" "$DATA_ROOT/spice_kernels/dawn_dynamic.tm" || true)
