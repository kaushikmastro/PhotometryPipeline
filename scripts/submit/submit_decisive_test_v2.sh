#!/usr/bin/env bash
#SBATCH --job-name=decisive_test_v2
#SBATCH --nodes=1 --ntasks=1 --cpus-per-task=2 --mem=8G
#SBATCH --partition=main --qos=standard --time=00:30:00
#SBATCH --output=logs/decisive_test_v2_%j.out --error=logs/decisive_test_v2_%j.err
...
echo "Node: $(hostname)"
python .tmp/decisive_test_v2_point_source.py
