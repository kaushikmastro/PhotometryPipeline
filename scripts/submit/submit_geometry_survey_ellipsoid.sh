...
# Input:  calibrated_raw_images/survey/ (845 F1B *IMG* files on disk)
...
IMG_DIR="${SCRATCH}/calibrated_raw_images/survey"
OUT_DIR="${SCRATCH}/geometry/ellipsoid/survey"
...
# ── Geometry grind ─────────────────────────────────────────────────────────
python scripts/geometry/run_geometry_survey_ellipsoid.py
...
""")
ELLIPSOID = "/scratch/kaushim07/vesta_data/geometry/ellipsoid/survey/*.parquet"
r = con.execute(f"""
  SELECT
    COUNT(*)                    AS n_total,
