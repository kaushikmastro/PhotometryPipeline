#!/usr/bin/env bash
#SBATCH --job-name=validate_sys_floor
#SBATCH --nodes=1 --ntasks=1 --cpus-per-task=2 --mem=8G
#SBATCH --partition=main --qos=standard --time=00:10:00
#SBATCH --output=logs/validate_sys_floor_%j.out --error=logs/validate_sys_floor_%j.err
...
echo "Node: $(hostname)"
python .tmp/validate_systematic_floor.py
