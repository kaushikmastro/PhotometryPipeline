"""
Usage:
    python scripts/extract_resolved_disk_hapke_ellipsoid.py
...
Input parquet patterns:
    data/04_geometry_tables_ellipsoid/survey/*.parquet  ← f_solar=892 (CORRECT)

Output parquet:
    data/06_silver_layer_ellipsoid/survey_ellipsoid_110825.parquet
"""
...
INPUT_GLOBS = ['data/04_geometry_tables_ellipsoid/survey/*.parquet']
OUTPUT_DIR = Path('data/06_silver_layer_ellipsoid')
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_PATH = OUTPUT_DIR / 'survey_ellipsoid.parquet'
...
def main() -> None:
    ...
    con.execute("PRAGMA threads=8;") # Match SLURM --cpus-per-task
    ...
    SELECT
        ...
        FROM read_parquet('data/04_geometry_tables_ellipsoid/survey/*.parquet')
