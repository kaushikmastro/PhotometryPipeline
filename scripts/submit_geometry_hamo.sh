...
#SBATCH --qos=standard
#SBATCH --output=logs/geometry_hamo_%j.out
#SBATCH --error=logs/geometry_hamo_%j.err
...
echo "========================================================"
echo "HAMO geometry grind — DSK/UNPRIORITIZED, f_solar=892"
echo "Start: $(date)"
...
grep -i "vesta_gaskell" /scratch/kaushim07/vesta_data/02_spice_kernels/dawn_dynamic.tm || \
    { echo "ERROR: no DSK found in metakernel"; exit 1; }

echo "DSK file size:"
stat -c '%s %n' /scratch/kaushim07/vesta_data/02_spice_kernels/vesta_gaskell_256*.bds || \
    { echo "ERROR: DSK file not found on scratch"; exit 1; }

echo "HAMO F1B image count:"
find /scratch/kaushim07/vesta_data/01_calibrated_images/hamo -name "*F1B*.IMG" | wc -l

python scripts/run_geometry_hamo.py
...
r = duckdb.sql('''
    ...
    FROM read_parquet(\"''' + hamo_dir + '''/*.parquet\")
''').fetchone()
...
sentinel = pathlib.Path('logs/hamo_geometry_complete.sentinel')
sentinel.write_text('OK')
print('HAMO GEOMETRY VALIDATED — sentinel written to', sentinel)
"