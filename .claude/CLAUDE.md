...
## Systematic Uncertainty Budget (no formal budget exists yet — this is the first documented item)

**theta_bar is shape-model-dependent, not just data-dependent.** The committed θ̄=8.2662°
(`geometry/gaskell_256/survey/`, Q=256 Gaskell SPC, confirmed via
`scripts/diagnostics/dsk_shape_resolution.py`: 786,432 plates, mean edge 1.686 km, mean
radius 262.12 km — see "VALIDATED PIPELINE STATE" below) is macroscopic roughness measured
*against a mesh with ~1.7 km mean plate size*. By construction it necessarily absorbs all
unresolved sub-plate topography as apparent roughness — a smoother-but-coarser mesh reads
more "roughness" into θ̄ than the same true surface would on a finer mesh, since real
topography below ~1.7 km scale has nowhere else to go in the model. A different shape
model, or a higher-Q Gaskell reconstruction of the *same* surface, would give a different
θ̄ for that reason alone, independent of any change in the photometric data. This is the
concrete instance of the "no systematic budget" gap in this pipeline (statistical/fit
uncertainties are tracked via `parameter_errors`/multi-start spread; mesh-resolution
sensitivity is not) and the right answer if anyone asks how sensitive θ̄ is to the shape
model: qualitatively real and expected, not yet quantified.

**Parked, not started**: Gaskell's **Q=512** SPC reconstruction of Vesta exists (this
project's kernel, `vesta_gaskell_256_110825.bds`, is the Q=256 *downsample* of it — see
"Why 'preliminary Gaskell DSK256'" below). Refitting Case 1 against the Q=512 mesh would
give a direct empirical measure of θ̄'s shape-model sensitivity (same surface, ~4x finer
plates, isolates the mesh-resolution effect from any data change). Obvious first item for
a systematic budget when one is built — do not act on this without being asked.
...
Production fit source: 04_geometry_tables_fast/survey/
  ...
  **`04_geometry_tables_fast/` no longer exists under that name (confirmed via `find`,
  Sep 18 2026) -- undocumented rename, not a deletion. The committed Case 1 fit
  (`logs/prelim_physfilter_25799987.out`) reads `data/04_geometry_tables_fast/survey/
  *F1B*.parquet`; the current `run_prelim_physfilter.py` reads
  `data/geometry/gaskell_256/survey/*F1B*.parquet` instead -- same 845 F1B parquets,
  same `committed_params.py` provenance note ("04_geometry_tables_fast"), same file
  layout/mtimes (May 23-24 2026). Treat `geometry/gaskell_256/survey/` as the current
  name of this directory for any future provenance check against the committed fit.
  No rename record exists anywhere in this repo (scripts, commits, or CLAUDE.md) --
  same untraceable-state failure class as the 7.669% figure below.**

  **29-frame contamination check (Sep 18 2026): Case 1 is uncontaminated.** The 29
  DOY-11123 frames misfiled under Survey (relocated to `calibrated_raw_images/
  approach/` earlier this project) are physically present in `geometry/gaskell_256/
  survey/` (part of the 845 F1B count above) but contribute **zero** pixels to the
  fit: 468 raw pixels total across all 29 files (Vesta unresolved at ~1.22M km range,
  phase=42.73°), max iof=0.0001 -- two orders of magnitude below the iof>0.01 shadow
  cut -- and `COUNT(*) FILTER (WHERE iof>0.01 AND incidence<50 AND emission<50) = 0`.
  None reach the 950 fit bins. w=0.46993/g=-0.33688/θ̄=8.2662° are unaffected; no
  re-fit needed. (A separate, unrelated set of 29 same-DOY stale parquets existed
  under `geometry/gaskell_dsk256_110825/survey/` -- this session's own relocation
  leftovers, not the ones checked here -- deleted Sep 18 2026, along with ~1.1GB of
  disposable scratch geometry dirs.)
...
4. **Refit Case 1 against Q=512 Gaskell** — see "Systematic Uncertainty Budget" above.
   Gaskell's Q=512 SPC reconstruction exists; the committed kernel
